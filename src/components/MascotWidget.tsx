...
const MascotWidget = () => {
  ...
  return <>
      ...
      <div className="fixed bottom-6 right-6 z-50 flex flex-col items-end gap-3">
        ...
        <AnimatePresence>
          ...
          damping: 25
              ...
              <div className="p-3 space-y-2">
                <button onClick={handleDevisClick} className="w-full flex items-center gap-3 px-4 py-3 rounded-xl bg-primary/5 hover:bg-primary/10 transition-colors group">
                ...
                <a href="tel:0465849498" className="w-full flex items-center gap-3 px-4 py-3 rounded-xl bg-secondary/5 hover:bg-secondary/10 transition-colors group" onClick={() => setIsOpen(false)}>
              ...
              <div className="px-5 py-3 bg-muted/30 border-t border-border">
                <p className="text-xs text-muted-foreground text-center">🚗 Intervention rapide partout à Marseille & région Paca.</p>
              </div>
            </motion.div>}
        </AnimatePresence>
        ...
        scale: 0.95,
        rotate: -5
      }} onClick={() => setIsOpen(!isOpen)} className="relative w-20 h-20 rounded-full bg-gradient-to-br from-primary to-primary/80 shadow-2xl border-4 border-white overflow-hidden group cursor-pointer" style={{
        boxShadow: '0 8px 32px rgba(255, 102, 0, 0.4), 0 0 0 4px rgba(255, 102, 0, 0.1)'
      }}>
          {/* Mascot Image */}
          <motion.img src={mascotImage} alt="Assistant Topglass" className="w-full h-full object-cover" animate={{
          y: [0, -3, 0]
        }} transition={{
          duration: 2,
          repeat: Infinity,
          ...
          {/* Pulse ring effect */}
          <motion.div className="absolute inset-0 rounded-full border-4 border-primary" animate={{
          scale: [1, 1.3, 1.3],
          opacity: [0.6, 0, 0]
        }} transition={{
          duration: 2,
